...
struct SensorBoost: AoCSolution {
	static func solve(filename: String) {
		print("\nDay 09 (Sensor Boost) -> \(filename)")
		...
		computer.input.append(1) // As per instructions
		//computer.debug = true
		computer.run(toOutput: false, resetPtr: true)
		//print(computer.output)
		
		let keycode = computer.output[0]
		
		print("Part One")
		print("The BOOST keycode is \(keycode)")
		
		computer.program = program
		computer.input.append(2) // As per instructions
		//computer.debug = true
		computer.run(toOutput: false, resetPtr: true)
		print(computer.output)
		
		let coords = computer.output[0]
	...
	static func runTests(filename: String) {
		print("\nDay 09 TEST (Sensor Boost) -> \(filename)")
		let input = AoCUtil.readInputFile(named: filename, removingEmptyLines: true)
		...
		// Outputs the large number in the middle of the program
			...
			while !bExit {
				...
				func readParams(count: Int) -> [Int] {
					...
					for i in 1...count {
						...
						if mode == .pos {
							p.append(readMem(readMem(ptr+1)))
						}
						else if mode == .rel {
