...
public abstract class Solution {
    ...
    public static Input challenge(Solution s) {
	return inputsForSolution(s).get(0);
    }
    
    public static List<Input> tests(Solution s) {
	var allInputs = inputsForSolution(s);
	List<Input> result = allInputs.stream().filter(i -> i.filename().contains("test")).toList();
	return result;
    }
