...
public class Day24 extends Solution {
    ...
    private String solvePartOne(Grid2D map) {
        ...
        while (!history.contains(map.getCoords())) {
            ...
            for (var coord : coords) {
                ...
                boolean nextIsBug = willHaveBug(isBug, adjacentWithBugs.size());
                if (nextIsBug) {
                    nextMap.set(coord, "#");
                }
            }
    ...
    private boolean willHaveBug(boolean isBug, int adjacentBugs) {
        if (isBug && adjacentBugs == 1) {
            // lives
            return true;
        } else if (isBug) {
            // dies
        } else if (!isBug && (adjacentBugs == 1 || adjacentBugs == 2)) {
            // infests
            return true;
        }
        return false;
    ...
    private String solvePartTwo(RecursiveGrid2D rMap) {
        ...
        for (int minute = 1; minute <= 200; minute++) {
            var zero = rMap.getGrid(0, true);
            var nextGrid = new RecursiveGrid2D(0, rMap.getDefaultValue(), rMap.getAdjacencyRule());

            calc(zero, nextGrid, -1);
            calc(zero, nextGrid, 1);
            
            rMap = nextGrid;
        }
        //rMap.print();
        return String.valueOf(rMap.bugCount());
    }
    
    private void calc(RecursiveGrid2D source, RecursiveGrid2D dest, int direction) {
        boolean bugsOnOuterPerimeter = false;
        boolean bugsOnInnerPerimeter = false;
        
        for (var coord : RecursiveGrid2D.iterableCoords) {
            //System.out.println(coord + " " + source.getLevel());
            boolean isBug = source.get(coord).equals("#");
            if (isBug) {
                if (source.coordIsOnOutsideBorder(coord)) {
                    bugsOnOuterPerimeter = true;
                }
                else if (source.coordIsOnInsideBorder(coord)) {
                    bugsOnInnerPerimeter = true;
                }
            }
            var adjCoords = source.getRAdjacent(coord);
            //System.out.println(adjCoords);
            int adjacentBugs = countAdjacentBugs(source, adjCoords);
            //System.out.println(adjacentBugs);
            boolean nextIsBug = willHaveBug(isBug, adjacentBugs);
            //System.out.println(nextIsBug);
            
            if (nextIsBug) {
                dest.set(coord, dest.getLevel(), "#");
            }
        }
        
        var nextLevel = source.getGrid(source.getLevel() + direction, false);
        if (nextLevel == null) {
            if ((direction < 0 && bugsOnOuterPerimeter) || (direction > 0 && bugsOnInnerPerimeter)) {
                nextLevel = source.getGrid(source.getLevel() + direction, true);
            }              
        }
        if (nextLevel != null) {
            calc(nextLevel, dest.getGrid(nextLevel.getLevel(), true), direction);
        }
    }
    
    private int countAdjacentBugs(RecursiveGrid2D rMap, List<Coord3D> coords) {
        int count = 0;
        for (var coord : coords) {
            var value = rMap.get(new Coord2D(coord.getX(), coord.getY()), coord.getZ());
            if (value.equals("#")) {
                count++;
            }
        }
...
class RecursiveGrid2D extends Grid2D {
    ...
    public static final List<Coord2D> iterableCoords;
    
    static {
        iterableCoords = outerExtent.getAllCoords();
        iterableCoords.remove(RecursiveGrid2D.center); // Never get neighbors of center
    }
    ...
    public List<Coord3D> getRAdjacent(Coord2D c) {
        ...
        //System.out.println("getRAdjacent " + c + " " + this.getLevel());
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (!getExtent().contains(n2d)) {
                ...
                Extent2D ext = null;
                if (i == 0) {
                    ext = new Extent2D(0, 4, 4, 4);
                } else if (i == 1) {
                    ext = new Extent2D(0, 0, 0, 4);
                } else if (i == 2) {
                    ext = new Extent2D(0, 0, 4, 0);
                } else {
                    ext = new Extent2D(4, 0, 4, 4);
                }
    ...
    public int bugCount() {
        ...
        while (rMap != null) {
            count += rMap.getCoordsWithValue("#").size();
            rMap = rMap.getInnerGrid();
        }
        return count;
    }
